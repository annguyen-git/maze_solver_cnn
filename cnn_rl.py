...
class MazeEnvironment:
    def __init__(self, maze_img_path):
        ...
        self.grid = self._process_maze_image()
        self.height, self.width = 9, 9 #(not in the mood to make this customizable)
    ...
    def _get_state(self):
        ...
        state = np.stack([
            state,
            np.ones((5, 5)) * (y - end_y) / 9,
            np.ones((5, 5)) * (x - end_x) / 9
        ], axis=-1)
    ...
    def step(self, action):
        ...
        if not (0 <= new_y < 9 and 0 <= new_x < 9):
        ...
        if self.current_pos == self.end_pos:
            ...
            return self._get_state(), 10 + (100 - len(self.path_history)), True, {}
...
def visualize_best_path(env, best_path):
    env.reset()
    env.path_history = []
    ...
    for pos in best_path:
        env.current_pos = pos
        env.path_history.append(pos)
        frames.append(env.render())
    
    save_frames_as_gif(frames, filename='optimal_path.gif')
    return frames
    
# Main function
def solve_maze(maze_img_path, training_episodes=10):
    env = MazeEnvironment(maze_img_path)
